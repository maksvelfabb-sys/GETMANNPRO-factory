...
with tabs[0]:
    ...
    for idx, row in display_df.iterrows():
        ...
        with c1:
            ...
            new_id = col_id.text_input("ID", value=str(row['ID']), key=f"id_{idx}")
            new_client = col_cl.text_input("Клієнт", value=str(row['Клієнт']), key=f"cl_{idx}")
            new_items = st.text_area("Товари", value=str(row['Товари']), key=f"it_{idx}", height=100)
            new_comm = st.text_input("Коментар", value=str(row['Коментар']), key=f"co_{idx}")
            
            # Логіка збереження змін тексту
            if (new_id != str(row['ID']) or new_client != str(row['Клієнт']) or 
                new_items != str(row['Товари']) or new_comm != str(row['Коментар'])):
                df.at[idx, 'ID'] = new_id
        ...
        with c2:
            ...
            if st.button("🔵 В роботу", key=f"bw_{idx}", use_container_width=True):
                df.at[idx, 'Готовність'] = "В роботі"; save_data(df); st.rerun()
            if st.button("🟢 Виконано", key=f"bd_{idx}", use_container_width=True):
                df.at[idx, 'Готовність'] = "Готово"; save_data(df); st.rerun()
            if st.button("⚪ В чергу", key=f"bq_{idx}", use_container_width=True):
                df.at[idx, 'Готовність'] = "В черзі"; save_data(df); st.rerun()
            
            st.markdown("---")
            # ВИПРАВЛЕНО KeyError: 'Sum' -> 'Сума'
            raw_sum = str(row['Сума']).replace(',', '.').split('.')[0]
            val_sum = int(raw_sum) if raw_sum.isdigit() else 0
            
            new_sum = st.number_input("Сума, грн", value=val_sum, key=f"sum_{idx}")
            if new_sum != val_sum:
...
with tabs[1]:
    st.subheader("Додати замовлення")
    with st.form("add"):
        f_id = st.text_input("ID")
        f_cl = st.text_input("Клієнт")
        f_it = st.text_area("Товари")
        f_sm = st.number_input("Сума", min_value=0)
        f_co = st.text_input("Коментар")
        if st.form_submit_button("Зберегти"):
            new_r = {'ID': f_id, 'Клієнт': f_cl, 'Товари': f_it, 'Сума': f_sm, 'Готовність': 'В черзі', 'Коментар': f_co}
