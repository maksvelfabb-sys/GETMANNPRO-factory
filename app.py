...
if 'df' not in st.session_state:
    st.session_state.df = load_csv(ORDERS_CSV_ID, COLS)
df = st.session_state.df

tabs = st.tabs(["📋 Журнал", "⚙️ Адмін"])

# --- ТАБ ЖУРНАЛ (Логіка без змін) ---
with tabs[0]:
    if can_edit:
    ...
    for idx, row in df_v.iterrows():
        with st.container(border=True):
            ...
            if role != "Токар":
                ...
                c_f1.metric("Сума", f"{round(total_sum, 2)} грн")
                c_f2.metric("Аванс", f"{avans} грн")
                c_f3.metric("Залишок", f"{round(total_sum - avans, 2)} грн")

            if can_edit:
                with st.expander("✏️ Редагувати"):
                    edited_df = st.data_editor(pd.DataFrame(items), num_rows="dynamic", key=f"ed_{idx}")
