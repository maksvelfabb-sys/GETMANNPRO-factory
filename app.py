...
# 3. Перевірка авторизації
# Використовуємо функцію з вашого модулю auth.py
if not check_auth():
    login_screen()
    st.stop()  # Зупиняємо виконання коду нижче, якщо користувач не увійшов

# --- ЯКЩО АВТОРИЗОВАНО, ВИКОНУЄТЬСЯ КОД НИЖЧЕ ---

# 4. Отримання даних користувача з сесії
user = st.session_state.auth
role = user.get('role', 'Користувач')
user_display = user.get('login') or user.get('email', 'Невідомий')
...
with st.sidebar:
    ...
    # Доступ до адмін-панелі лише для Адмінів
    if role in ["Адмін", "Супер Адмін"]:
        menu_options.append("🔐 Адмін-панель")
    ...
    # Кнопка виходу (викликає вашу функцію logout з куками)
    if st.button("🚪 Вийти з системи", use_container_width=True):
        logout()
...
if menu == "📋 Замовлення":
    ...
    # Розподіляємо функціонал на вкладки, щоб уникнути Duplicate ID
    tab_view, tab_create = st.tabs(["🔎 Журнал замовлень", "➕ Створити нове"])
    
    with tab_view:
        # Модуль відображення та живого редагування карток
        show_order_cards()
        
    with tab_create:
        # Модуль форми створення нового замовлення
        show_create_order()

elif menu == "🔐 Адмін-панель":
    # Модуль керування базою користувачів та очищення даних
    show_admin_panel()
